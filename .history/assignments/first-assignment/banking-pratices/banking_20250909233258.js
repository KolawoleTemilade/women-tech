...
if (account2.type === "savings") {
    console.log (`interest balance in a year is ${account1.currency +" " + account2.balance * rate * time}`)
    console.log (`the new balance for ${account2.name} with interest is now ${account2.balance * rate * time + account2.balance}`)
} else { 
    console.log (account1.balance + deposit - withdawal)
};
...
if (account4.type === "savings") {
    ...
    console.log (account4.balance + deposit - withdawal) //so for this, i was thinking if i should multiply the deposit by the number of days(30) and also the withdrawal, but im not too certain as deposit and withdrawals are not made everyday
}