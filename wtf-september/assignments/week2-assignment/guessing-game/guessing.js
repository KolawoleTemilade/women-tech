...
const baskets = [ //this is an array of objects,storing each baskets objects
    ...
    function checkBasket(basketNumber) { //we have a function here so we can reuse this logic anytime.
            let basket = baskets[basketNumber -1]    //now i am going through each fruit baskets and i'm saving them inside basket variable
            if (basket.hasFruit){ //now i have a conditional statement that if maybe basket 1 has fruits
                console.log (basket.name + " " + "has fruits") // then it should get me the name of the basket and tell me it has fruits
                firstBasket = true;  // while i use this my boolean to compare if it is indeed true or false
            } else {     //or else
                console.log(basket.name + " " + "is empty") // it should just do its thing and return whatever answer it gets
            }
// prompt("you have 3 choices : Basket 1 , Basket 2, or Basket 3. " + "choose one fruit basket");
    }
    checkBasket(1)  // this is where my reusable function calls everything out , basket by basket
    checkBasket(2)
    checkBasket(3)
